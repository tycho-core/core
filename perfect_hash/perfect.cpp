...
// Created : Tuesday, 8 April 2008 9:51:06 PM
// ------------------------------------------------------------------------------
// perfect.c: code to generate code for a hash for perfect hashing.
// (c) Bob Jenkins, September 1996, December 1999
// You may use this code in any way you wish, and it is core::allocator::free.  No warranty.
...
static void checkdup(key *key1, key *key2, hashform *form)
{
  ...
  case hashform::STRING_HT:
	...
	!core::mem_cmp(key1->name_k, key2->name_k, (size_t)key1->len_k))
    {
 ...
 */
static int inittab(bstuff   *tabb, ub4 blen, key *keys, hashform *form, int complete)
...
// int       complete;        /* TRUE means to complete init despite collisions */
{
  ...
  core::mem_zero((void *)tabb, (size_t)(sizeof(bstuff)*blen));
...
static void initnorm(key *keys, ub4 alen, ub4 blen, ub4 smax, ub4 salt, gencode *final)
...
// gencode  *final;                          /* output, code for the final hash */
{
  ...
  {
	final->p_type = gencode::CT_NORM_GT_UB4BITS;
  ...
  else
  {
    ...
    if (smax <= 1)
    {
		final->p_type = gencode::CT_ZERO;
		sprintf(final->line[1], "  rsl = 0;\n");
    }
    else if (blen < USE_SCRAMBLE)
    {
		final->p_type = gencode::CT_NORM_LE_UB4BITS_SCRAMBLED;
      sprintf(final->line[1], "  rsl = ((val>>%ld)^tab[val&0x%lx]);\n",
	      UB4BITS-mylog2(alen), blen-1);
    }
    else
    {
		final->p_type = gencode::CT_NORM_LE_UB4BITS;
      sprintf(final->line[1], "  rsl = ((val>>%ld)^scramble[tab[val&0x%lx]]);\n",
	      UB4BITS-mylog2(alen), blen-1);
    }

	// record the arguments for this function
	final->p_shift = UB4BITS-mylog2(alen);
	final->p_mask = blen-1;
	final->p_initlev = initlev;
  }
}
...
static void initinl(key *keys, ub4 alen, ub4 blen, ub4 smax, ub4 salt, gencode *final)
// key      *keys;                                          /* list of all keys */
// ub4       alen;                    /* (a,b) has a in 0..alen-1, a power of 2 */
// ub4       blen;                    /* (a,b) has b in 0..blen-1, a power of 2 */
...
static void duplicates(bstuff   *tabb, ub4 blen, key *keys, hashform *form)
	...
	)
{
  ...
  int  trans = (form->speed == hashform::SLOW_HS || form->perfect == hashform::MINIMAL_HP);
	...
	hashform *form)
{
  ...
  core::mem_set((void *)tabh, 0, 
	 (size_t)(sizeof(hstuff)*
		  ((form->perfect == hashform::MINIMAL_HP) ? nkeys : smax)));
  core::mem_set((void *)tabq, 0, (size_t)(sizeof(qstuff)*(blen+1)));
		...
		hashform *form)                                           /* user directives */
{
  ...
  *tabb = (bstuff *)core::allocator::malloc((size_t)(sizeof(bstuff)*(*blen))); 
  tabq  = (qstuff *)remalloc(sizeof(qstuff)*(*blen+1));
  tabh  = (hstuff *)remalloc(sizeof(hstuff)*(form->perfect == hashform::MINIMAL_HP ? nkeys : *smax));
	...
	hashform *form)                                           /* user directives */
{
  ...
  *tabb = (bstuff *)remalloc((size_t)(sizeof(bstuff)*(*blen)), "perfect.c, tabb");
  ...
  tabh  = (hstuff *)remalloc(sizeof(hstuff)*(form->perfect == hashform::MINIMAL_HP ? nkeys : *smax), "perfect.c, tabh");
  ...
  *salt = 0;
  ...
  for (trysalt=1; ; ++trysalt)
  {
	...
	{
	  *blen *= 2;
	  core::allocator::free(tabq);
	  core::allocator::free(*tabb);
	  *tabb  = (bstuff *)core::allocator::malloc((size_t)(sizeof(bstuff)*(*blen)));
	  tabq  = (qstuff *)core::allocator::malloc((size_t)(sizeof(qstuff)*(*blen+1)));
	}
	else
	{
	...
	hashform  *form)                                          /* user directives */	
{
  ...
  *nkeys = 0;
  while (fgets(mytext, MAXKEYLEN, stdin))
  {
    ...
    if (form->mode == hashform::AB_HM)
    {
      sscanf(mytext, "%lx %lx ", &mykey->a_k, &mykey->b_k);
    }
    else if (form->mode == hashform::ABDEC_HM)
    {
      sscanf(mytext, "%ld %ld ", &mykey->a_k, &mykey->b_k);
    }
    else if (form->mode == hashform::HEX_HM)
    {
      sscanf(mytext, "%lx ", &mykey->hash_k);
    }
    else if (form->mode == hashform::DECIMAL_HM)
    {
...
char **argv;
{
  ...
  case 2:
	...
	form.mode = INLINE_HM;  form.hashtype = STRING_HT; break;
      case 'h': case 'H':
	form.mode = hashform::HEX_HM;     form.hashtype = hashform::INT_HT; break;
      case 'd': case 'D':
	form.mode = hashform::DECIMAL_HM; form.hashtype = hashform::INT_HT; break;
      case 'a': case 'A':
	form.mode = hashform::AB_HM;      form.hashtype = hashform::AB_HT; break;
      case 'b': case 'B':
	form.mode = hashform::ABDEC_HM;   form.hashtype = hashform::AB_HT; break;
      }
      mode_given = TRUE;
      break;
    case 'm': case 'M':
	...
	usage_error();
      ...
      {
      case 'p': case 'P':
	form.perfect = hashform::NORMAL_HP; break;
      case 'm': case 'M':
	form.perfect = hashform::MINIMAL_HP; break;
      }
      minimal_given = TRUE;
      break;
    case 'f': case 'F':
	...
	usage_error();
      ...
      {
      case 'f': case 'F':
	form.speed = hashform::FAST_HS; break;
      case 's': case 'S':
